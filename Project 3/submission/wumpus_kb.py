...
CONDITIONAL = ' >> '
BICONDITIONAL = ' <=> '
DIRECTIONS = {'Same': (0, 0), 'North': (0, 1), 'East': (1, 0), 'South': (0, -1), 'West': (-1, 0)}
OPPOSITE_DIRECTIONS = {'East': 'West', 'West': 'East', 'North': 'South', 'South': 'North'}
ALL_LOCATIONS = lambda x1, x2, y1, y2: set([(x, y) for x in range(x1, x2 + 1) for y in range(y1, y2+1)])
...
def axiom_generator_percept_sentence(t, tvec):
    ...
    """
    lookup = proposition_bases_perceptual_fluents
    axiom_str = ''
    "*** YOUR CODE HERE ***"
    axiom_str = AND.join([f'{NOT if not percept else ""}{lookup[i]}{t}' for i, percept in enumerate(tvec)])
    return axiom_str
...
def axiom_generator_initial_location_assertions(x, y):
    ...
    axiom_str = ''
    "*** YOUR CODE HERE ***"
    axiom_str = AND.join([NOT + pit_str(x, y), NOT + wumpus_str(x, y)])
    return axiom_str
...
def axiom_generator_pits_and_breezes(x, y, xmin, xmax, ymin, ymax):
    ...
    for dx, dy in DIRECTIONS.values():
        if (xmin <= (x + dx) <= xmax) and (ymin <= (y + dy) <= ymax):
            li.append(pit_str(x+dx, y+dy))

    axiom_str = BICONDITIONAL.join([f"({breeze_str(x, y)})", f"({OR.join(li)})"])
    return axiom_str
...
def axiom_generator_wumpus_and_stench(x, y, xmin, xmax, ymin, ymax):
    ...
    for dx, dy in DIRECTIONS.values():
        if (xmin <= x + dx <= xmax) and (ymin <= y + dy <= ymax):
            li.append(wumpus_str(x + dx, y + dy))

    axiom_str = BICONDITIONAL.join([stench_str(x, y), f"({OR.join(li)})"])
    return axiom_str
...
def axiom_generator_at_least_one_wumpus(xmin, xmax, ymin, ymax):
    ...
    axiom_str = ''
    "*** YOUR CODE HERE ***"
    axiom_str = OR.join([wumpus_str(x, y) for x in range(xmin, xmax+1) for y in range(ymin, ymax+1)])
    return axiom_str
...
def axiom_generator_at_most_one_wumpus(xmin, xmax, ymin, ymax):
    ...
    "*** YOUR CODE HERE ***"
    li = []
    all_locations = ALL_LOCATIONS(xmin, xmax, ymin, ymax)
    for x, y in all_locations:
        no_wumpus_other_loc = AND.join([NOT + wumpus_str(x2, y2) for x2, y2 in (all_locations - {(x, y)})])
        li.append(f'({BICONDITIONAL.join([wumpus_str(x, y), f"({no_wumpus_other_loc})"])})')
    axiom_str = AND.join(li)
    return axiom_str
...
def axiom_generator_only_in_one_location(xi, yi, xmin, xmax, ymin, ymax, t = 0):
    ...
    axiom_str = ''
    "*** YOUR CODE HERE ***"
    other_locs = ALL_LOCATIONS(xmin, xmax, ymin, ymax) - {(xi, yi)}
    not_in_other_locs = AND.join([NOT + state_loc_str(x, y, t) for x, y in other_locs])
    axiom_str = AND.join([state_loc_str(xi, yi, t), not_in_other_locs])
    return axiom_str
...
def axiom_generator_only_one_heading(heading = 'north', t = 0):
    ...
    axiom_str = ''
    "*** YOUR CODE HERE ***"
    head_dirs = {'north', 'east', 'south', 'west'}
    not_head_other = AND.join([f"{NOT}Heading{h.title()}{t}" for h in head_dirs - {heading}])
    axiom_str = AND.join([f"Heading{heading.title()}{t}", not_head_other])
    return axiom_str
...
def axiom_generator_have_arrow_and_wumpus_alive(t = 0):
    ...
    axiom_str = ''
    "*** YOUR CODE HERE ***"
    axiom_str = AND.join([state_have_arrow_str(t), state_wumpus_alive_str(t)])
    return axiom_str
...
def axiom_generator_location_OK(x, y, t):
    ...
    axiom_str = ''
    "*** YOUR CODE HERE ***"
    wumpus_no_danger = f"({OR.join([NOT + state_wumpus_alive_str(t), NOT + wumpus_str(x, y)])})"
    other = f"({AND.join([wumpus_no_danger, NOT + pit_str(x, y)])})"
    axiom_str = BICONDITIONAL.join([state_OK_str(x, y, t), other])
    return axiom_str
...
def axiom_generator_breeze_percept_and_location_property(x, y, t):
    ...
    axiom_str = ''
    "*** YOUR CODE HERE ***"
    lhs = f'({AND.join([state_loc_str(x, y, t), percept_breeze_str(t)])})'
    one = f'({CONDITIONAL.join([lhs, breeze_str(x, y)])})'

    lhs = f'({AND.join([state_loc_str(x, y, t), NOT + percept_breeze_str(t)])})'
    two = f'({CONDITIONAL.join([lhs, NOT + breeze_str(x, y)])})'

    axiom_str = AND.join([one, two])
    return axiom_str
...
def axiom_generator_stench_percept_and_location_property(x, y, t):
    ...
    axiom_str = ''
    "*** YOUR CODE HERE ***"
    lhs = f'({AND.join([state_loc_str(x, y, t), percept_stench_str(t)])})'
    one = f'({CONDITIONAL.join([lhs, stench_str(x, y)])})'

    lhs = f'({AND.join([state_loc_str(x, y, t), NOT + percept_stench_str(t)])})'
    two = f'({CONDITIONAL.join([lhs, NOT + stench_str(x, y)])})'
...
def axiom_generator_at_location_ssa(t, x, y, xmin, xmax, ymin, ymax):
    ...
    for direction in DIRECTIONS.keys():
        if direction == "Same":
            same_loc_actions = ['Wait', 'Bump', 'Grab', 'Shoot', 'TurnLeft', 'TurnRight']
            for a in same_loc_actions:
                time = t + 1 if a == 'Bump' else t
                lhs = f'({AND.join([state_loc_str(x, y, t), f"{a}{time}", NOT + action_forward_str(t)])})'
                li.append(f'({CONDITIONAL.join([lhs, state_loc_str(x, y, t+1)])})')
        else:
            dx, dy = DIRECTIONS[direction]
            opp = OPPOSITE_DIRECTIONS[direction]
            if (xmin <= x + dx <= xmax) and (ymin <= y + dy <= ymax):
                lhs = f'({AND.join([state_loc_str(x+dx, y+dy, t), f"Heading{opp}{t}", action_forward_str(t)])})'
                li.append(f'({CONDITIONAL.join([lhs, state_loc_str(x, y, t + 1)])})')
...
def axiom_generator_have_arrow_ssa(t):
    ...
    axiom_str = ''
    "*** YOUR CODE HERE ***"
    rhs = f'({AND.join([state_have_arrow_str(t), NOT + action_shoot_str(t)])})'
    axiom_str = BICONDITIONAL.join([state_have_arrow_str(t+1), rhs])
    return axiom_str
...
def axiom_generator_wumpus_alive_ssa(t):
    ...
    axiom_str = ''
    "*** YOUR CODE HERE ***"
    rhs = f'({AND.join([state_wumpus_alive_str(t), NOT + percept_scream_str(t+1)])})'
    axiom_str = BICONDITIONAL.join([state_wumpus_alive_str(t + 1), rhs])
    return axiom_str
...
def axiom_generator_heading_north_ssa(t):
    ...
    axiom_str = ''
    "*** YOUR CODE HERE ***"
    actions = f'({AND.join([NOT + action_turn_left_str(t), NOT + action_turn_right_str(t)])})'
    already = f'({AND.join([state_heading_north_str(t), actions])})'

    turn_right = f'({AND.join([state_heading_west_str(t), action_turn_right_str(t)])})'
    turn_left = f'({AND.join([state_heading_east_str(t), action_turn_left_str(t)])})'

    rhs = f'({OR.join([already, turn_left, turn_right])})'
    axiom_str = BICONDITIONAL.join([state_heading_north_str(t + 1), rhs])
    return axiom_str
...
def axiom_generator_heading_east_ssa(t):
    ...
    "*** YOUR CODE HERE ***"
    actions = f'({AND.join([NOT + action_turn_left_str(t), NOT + action_turn_right_str(t)])})'
    already = f'({AND.join([state_heading_east_str(t), actions])})'

    turn_right = f'({AND.join([state_heading_north_str(t), action_turn_right_str(t)])})'
    turn_left = f'({AND.join([state_heading_south_str(t), action_turn_left_str(t)])})'

    rhs = f'({OR.join([already, turn_left, turn_right])})'
    axiom_str = BICONDITIONAL.join([state_heading_east_str(t + 1), rhs])
    return axiom_str
...
def axiom_generator_heading_south_ssa(t):
    ...
    "*** YOUR CODE HERE ***"
    actions = f'({AND.join([NOT + action_turn_left_str(t), NOT + action_turn_right_str(t)])})'
    already = f'({AND.join([state_heading_south_str(t), actions])})'

    turn_right = f'({AND.join([state_heading_east_str(t), action_turn_right_str(t)])})'
    turn_left = f'({AND.join([state_heading_west_str(t), action_turn_left_str(t)])})'

    rhs = f'({OR.join([already, turn_left, turn_right])})'
    axiom_str = BICONDITIONAL.join([state_heading_south_str(t + 1), rhs])
    return axiom_str
...
def axiom_generator_heading_west_ssa(t):
    ...
    "*** YOUR CODE HERE ***"
    actions = f'({AND.join([NOT + action_turn_left_str(t), NOT + action_turn_right_str(t)])})'
    already = f'({AND.join([state_heading_west_str(t), actions])})'

    turn_right = f'({AND.join([state_heading_south_str(t), action_turn_right_str(t)])})'
    turn_left = f'({AND.join([state_heading_north_str(t), action_turn_left_str(t)])})'

    rhs = f'({OR.join([already, turn_left, turn_right])})'
    axiom_str = BICONDITIONAL.join([state_heading_west_str(t + 1), rhs])
    return axiom_str
...
def axiom_generator_heading_only_north(t):
    ...
    axiom_str = ''
    "*** YOUR CODE HERE ***"
    directions = DIRECTIONS.keys() - {'Same', 'North'}
    rhs = f"({AND.join([NOT + f'Heading{direction}{t}' for direction in directions])})"
    axiom_str = BICONDITIONAL.join([state_heading_north_str(t), rhs])
    return axiom_str
...
def axiom_generator_heading_only_east(t):
    ...
    axiom_str = ''
    "*** YOUR CODE HERE ***"
    directions = DIRECTIONS.keys() - {'Same', 'East'}
    rhs = f"({AND.join([NOT + f'Heading{direction}{t}' for direction in directions])})"
    axiom_str = BICONDITIONAL.join([state_heading_east_str(t), rhs])
    return axiom_str
...
def axiom_generator_heading_only_south(t):
    ...
    axiom_str = ''
    "*** YOUR CODE HERE ***"
    directions = DIRECTIONS.keys() - {'Same', 'South'}
    rhs = f"({AND.join([NOT + f'Heading{direction}{t}' for direction in directions])})"
    axiom_str = BICONDITIONAL.join([state_heading_south_str(t), rhs])
    return axiom_str
...
def axiom_generator_heading_only_west(t):
    ...
    axiom_str = ''
    "*** YOUR CODE HERE ***"
    directions = DIRECTIONS.keys() - {'Same', 'West'}
    rhs = f"({AND.join([NOT + f'Heading{direction}{t}' for direction in directions])})"
    axiom_str = BICONDITIONAL.join([state_heading_west_str(t), rhs])
    return axiom_str
...
def axiom_generator_only_one_action_axioms(t):
    ...
    "*** YOUR CODE HERE ***"
    li = []
    for action in proposition_bases_actions:
        rhs = AND.join([f'{NOT}{a}{t}' for a in set(proposition_bases_actions) - {action}])
        li.append(f"({BICONDITIONAL.join([f'({action}{t})', f'({rhs})'])})")
