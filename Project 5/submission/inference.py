...
class DiscreteDistribution(dict):
    ...
    def normalize(self):
        ...
        totalSum = self.total()
        if totalSum:
            self.update((k, float(v) / float(totalSum)) for k, v in self.items())
    ...
    def sample(self):
        ...
        copyDist, randomVal, limit = DiscreteDistribution(dict.copy(self)), random(), 0
        copyDist.normalize()
        for k, v in copyDist.items():
            if v and (limit <= randomVal < limit + v):
                return k
            limit += v
...
class InferenceModule:
    ...
    def getObservationProb(self, noisyDistance, pacmanPosition, ghostPosition, jailPosition):
        ...
        obsProb = busters.getObservationProbability(noisyDistance, trueDistance)
        return obsProb
    ...
    def elapseTime(self, gameState):
        ...
        for oldPos in self.allPositions:
            ...
            for newPos in newPosDist:
                self.beliefs[newPos] += newPosDist[newPos] * beliefsCopy[oldPos]
...
class ParticleFilter(InferenceModule):
    ...
    def initializeUniformly(self, gameState):
        ...
        # dist = DiscreteDistribution(Counter(self.legalPositions))
        # self.particles += [dist.sample() for _ in range(self.numParticles)]
        n = len(self.legalPositions)
        for p in self.legalPositions:
            self.particles += ([p] * (self.numParticles // n))
    ...
    def observeUpdate(self, observation, gameState):
        ...
        for pos in self.particles:
            beliefs[pos] += self.getObservationProb(observation, pacPos, pos, jailPos)
        if not beliefs.total():
            self.initializeUniformly(gameState)
        else:
    ...
    def getBeliefDistribution(self):
        ...
        "*** YOUR CODE HERE ***"
        from collections import Counter
        beliefDist = DiscreteDistribution(Counter(self.particles))
